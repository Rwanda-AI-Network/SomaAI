        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    for start_page in range(1, total_pages + 1, batch_size):
                        ...
                        with ThreadPoolExecutor(max_workers=max_workers) as executor:
                            results = list(
                                executor.map(
                                    process_image,
                                    enumerate(batch_images, start=start_page),
                                )
                            )
                    ...
                    logger.info(
                        f"PDF processing failed ({pdf_err}), trying single image"
                    )
                    image = Image.open(tmp_path)
                    text = pytesseract.image_to_string(image, lang=language)
                    pages_data = [
                        Page(page_number=1, content=text, metadata={"method": "ocr"})
                    ]
            ...
            logger.info(
                f"OCR extraction successful. Extracted {len(pages_data)} pages."
            )
