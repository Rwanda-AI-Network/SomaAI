...
class CellNode:
    ...
    parent_id: tuple[int, int] | None = None
...
class Grid:
    ...
    cells: list[list[CellNode | None]]
    width: int
    height: int
    uf: UnionFind

    def __getitem__(self, pos: tuple[int, int]) -> CellNode | None:
        r, c = pos
    ...
    def get_merged_owner(self, r: int, c: int) -> CellNode | None:
...
class TableNormalizer:
    ...
    def normalize(raw_table: list[list[Any]]) -> Grid:
        ...
        cells: list[list[CellNode | None]] = [[None] * width for _ in range(height)]
        ...
        for r, row in enumerate(raw_table):
            for c, value in enumerate(row):
                ...
                if (
                    c > 0
                    and cells[r][c - 1]
                    and cells[r][c - 1].content == content
                    and content
                ):
                ...
                if (
                    r > 0
                    and cells[r - 1][c]
                    and cells[r - 1][c].content == content
                    and content
                ):
...
class HeaderExtractor:
    ...
    def build_header_tree(grid: Grid) -> dict[int, str]:
        ...
        for col_idx in range(grid.width):
            ...
            if not path_parts:
                col_name = HeaderExtractor._infer_column_name(
                    grid, col_idx, header_row_count
                )
...
class RAGSerializer:
    ...
    def serialize(grid: Grid, col_paths: dict[int, str]) -> dict[str, Any]:
        ...
        for row in data_rows:
            ...
            for key, value in list(row.items())[1:]:
                if value:
                    triples.append(
                        {"subject": entity, "predicate": key, "object": value}
                    )
    ...
    def _to_markdown(grid: Grid, col_paths: dict[int, str], header_rows: int) -> str:
        ...
        # Header
        header_items = [col_paths.get(c, f"Col{c}") for c in range(grid.width)]
