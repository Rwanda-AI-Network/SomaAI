...
class BM25Index:
    ...
    def add_documents(self, texts: list[str], doc_ids: list[str]) -> None:
        ...
        with self._lock:
            ...
            should_rebuild = (
                time_since_rebuild > self._rebuild_interval
                or len(self._pending_updates) >= self._rebuild_threshold
            )
            ...
            if should_rebuild:
                ...
                t_type = (
                    "time"
                    if time_since_rebuild > self._rebuild_interval
                    else "threshold"
                )
                msg = (
                    f"BM25 index rebuilt. Total: {len(self._corpus)} "
                    f"documents (triggered by {t_type})"
                )
                logger.info(msg)
            else:
                # Deferred rebuild
                logger.debug(
                    f"BM25 update deferred. Pending: "
                    f"{len(self._pending_updates)} documents, "
    ...
    def force_rebuild(self) -> None:
        ...
        with self._lock:
            if self._pending_updates:
                ...
                self._last_rebuild = time.time()
                logger.info(
                    f"BM25 index force rebuilt. Total: {len(self._corpus)} documents"
                )
