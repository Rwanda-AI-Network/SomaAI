...
if TYPE_CHECKING:
    from somaai.utils.text_extractor import ExtractionResult, Section, Table, Page
...
class SemanticChunker:
    ...
    def chunk(
        ...
        if extraction.hierarchy:
            ...
            logger.info(f"No hierarchy detected, chunking {len(extraction.pages)} pages")
    ...
    def _split_large_section(
        ...
        for i, text_chunk in enumerate(text_chunks):
            ...
            # Note: We do this AFTER splitting to ensure the split is based on content size,
            # but we prepend the title so the vector model sees it.
            # This might slightly exceed max_chunk_size, which is acceptable for metadata.
    ...
    def _chunk_pages(
        ...
        for page in pages:
            ...
            if len(content) <= self.max_chunk_size:
                ...
                chunks.extend(
                    self._split_large_page(page, base_metadata)
                )
