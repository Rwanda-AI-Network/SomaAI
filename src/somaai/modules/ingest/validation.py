...
class ExtractionValidator:
    ...
    MIN_UNIQUE_CHARS = 20
    CURRICULUM_MARKERS = ["chapter", "section", "lesson", "exercise", "question", "answer"]
    ...
    def validate(self, extraction: ExtractionResult) -> ValidationReport:
        ...
        if unique_chars < self.MIN_UNIQUE_CHARS:
            issues.append(ValidationIssue(
                severity="critical",
                message=f"Low character diversity ({unique_chars} unique characters)",
    ...
    def validate(self, chunks: list[Document]) -> ValidationReport:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if content_length == 0:
                ...
                issues.append(ValidationIssue(
                    severity="warning",
                    message=f"Very short chunk ({content_length} chars): {chunk_id}",
        ...
        try:
            ...
            if not results:
                ...
                if chunk.metadata.get("doc_id") != doc_id:
                    issues.append(ValidationIssue(
                        severity="critical",
                        message=f"doc_id mismatch: expected {doc_id}, got {chunk.metadata.get('doc_id')}",
