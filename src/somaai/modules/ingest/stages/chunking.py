...
class ChunkingStage(PipelineStage):
    ...
    async def execute(self, ctx: PipelineContext) -> StageResult:
        ...
        base_metadata = {
            ...
            "extraction_method": ctx.extraction_result.metadata.get(
                "method", "structured"
            ),
        ...
        ctx.chunks = await asyncio.to_thread(
            self.chunker.chunk, ctx.extraction_result, base_metadata
        )
        ...
        validation = await asyncio.to_thread(self.validator.validate, ctx.chunks)
