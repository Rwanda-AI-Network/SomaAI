...
from somaai.modules.ingest.exceptions import EmbeddingError, StorageError
...
class VectorStorageStage(PipelineStage):
    ...
    def __init__(self, store: QdrantStore, batch_size: int = 50, max_retries: int = 3):
    ...
    async def execute(self, ctx: PipelineContext) -> StageResult:
        ...
        try:
            ...
            for batch in self._batch_chunks(ctx.chunks):
                ...
                # Track stored IDs
                ctx.stored_chunk_ids.extend(c.metadata["chunk_id"] for c in batch)

                batch_idx += 1
                progress = batch_idx / total_batches
                self._report_progress(
                    ctx, f"Batch {batch_idx}/{total_batches}", progress
                )
