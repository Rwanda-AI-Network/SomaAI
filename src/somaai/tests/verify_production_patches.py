...
class TestProductionHardening(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_groq_llm_retries(self):
        ...
        mock_client.chat.completions.create.side_effect = [
            ...
            MagicMock(choices=[MagicMock(message=MagicMock(content='{"answer": "success"}'))]),
        ]
    ...
    async def test_rag_pipeline_retrieval_timeout_fallback(self):
        """Verify RAG pipeline falls back to insufficient_context on retrieval timeout."""
        pipeline = RAGPipeline(settings)
        pipeline.retriever = MagicMock()
        
        async def slow_retrieve(*args, **kwargs):
            await asyncio.sleep(5) # Shorter but still enough to trigger if we lower timeout for test
            return [], ""
        ...
        with patch("somaai.modules.rag.pipelines.sanitize_query", return_value="test"), \
             patch("somaai.modules.rag.pipelines.classify_query", return_value=("curriculum", None)), \
            ...
            self.assertTrue("couldn't find relevant curriculum content" in result["answer"].lower())
    ...
    async def test_rag_pipeline_generation_timeout_fallback(self):
        ...
        with patch("somaai.modules.rag.pipelines.sanitize_query", return_value="test"), \
            ...
            result = await pipeline.run("test query")
            self.assertTrue("unable to provide a detailed answer" in result["answer"].lower())
