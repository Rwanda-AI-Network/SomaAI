...
class TestSecurityHardening:
    ...
    def test_actor_isolation_messages(self, client: TestClient):
        """CRITICAL: Actor B must not access Actor A's data."""
        ...
        try:
            ...
            resp = client.get(
                f"/api/v1/chat/conversations/{convo_a['id']}/messages/{msg_a['message_id']}"
            )
            assert resp.status_code == 404

            # Try to get Actor A's citations
            resp = client.get(
                f"/api/v1/chat/conversations/{convo_a['id']}/messages/{msg_a['message_id']}/citations"
            )
            assert resp.status_code == 404
    ...
    def test_cross_conversation_leakage(self, client: TestClient):
        """CRITICAL: Message from convo_1 must not be accessible via convo_2."""
        ...
        resp = client.get(
            f"/api/v1/chat/conversations/{convo2['id']}/messages/{msg1['message_id']}"
        )
        assert resp.status_code == 404
...
class TestPaginationHardening:
    ...
    def test_message_history_pagination(self, client: TestClient):
        ...
        # Page 2
        resp = client.get(
            f"/api/v1/chat/conversations/{convo['id']}/messages?limit=2&cursor={cursor}"
        )
        data = resp.json()
        assert len(data["messages"]) == 2
...
class TestResilienceHardening:
    ...
    def test_rag_exception_fallback_behavior(self, mock_run, client: TestClient):
        """Verify that a pipeline crash results in a 201 with fallback text."""
    ...
    def test_rag_api_timeout_504(self, mock_ask, client: TestClient):
        """Verify service timeout triggers a 504 in the API layer."""
        mock_ask.side_effect = asyncio.TimeoutError()
        convo = _create_conversation(client)

        resp = client.post(
            f"/api/v1/chat/conversations/{convo['id']}/ask", json={"question": "Slow?"}
        )
    ...
    def test_invalid_cursor_encoding_fails_gracefully(self, client: TestClient):
        ...
        resp = client.get(
            f"/api/v1/chat/conversations/{convo['id']}/messages?cursor=not-base64-junk"
        )
