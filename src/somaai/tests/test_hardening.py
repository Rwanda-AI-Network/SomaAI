...
def setup_metadata():
    ...
    async def _seed():
        ...
        async with async_session_maker() as db:
            try:
                # Grades
                db.add(Grade(id="S1", name="Senior 1", level="secondary", display_order=1))
                db.add(Grade(id="S2", name="Senior 2", level="secondary", display_order=2))
                # Subjects
...
class TestHardening:
    ...
    def test_actor_isolation(self, client: TestClient):
        ...
        resp_a = client.post("/api/v1/chat/conversations", json={"grade": "S1", "subject": "science"})
        ...
        # Should be 403 Forbidden or 404 Not Found (current implementation uses 403 or 404 via get_owned)
    ...
    def test_list_conversations_filter_isolation(self, client: TestClient):
        ...
        client.post("/api/v1/chat/conversations", json={"grade": "S1", "subject": "science"})
        client.post("/api/v1/chat/conversations", json={"grade": "S2", "subject": "math"})
    ...
    def test_case_sensitivity_on_metadata(self, client: TestClient):
        ...
        resp = client.post("/api/v1/chat/conversations", json={"grade": "s1", "subject": "SCIENCE"})
